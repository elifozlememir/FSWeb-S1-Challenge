...
function fenomenEkle(arr, number, profile, follower, posts, platform) {
  let newarr = [...arr];
...
function enFenomenler(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (Number(arr[i].followers) > 100000000) {
      newarr.push(arr[i].profile);
    }
  }
  return newarr;
...
function fenomenGonderimSayisi(arr, profilname) {

  for (let i = 0; i < arr.length; i++) {
    if (arr[i].profile == profilname) {
      return arr[i].posts;
    }
  }
  return;
...
function platformaGoreCokGonderiYapanFenomen(arr, plat) {
  let maxpost = 0;
  let name = "";

  for (let i = 0; i < arr.length; i++) {

    if (arr[i].platform == plat) {

      if (arr[i].posts > maxpost && arr[i].posts != 'NA') {
        maxpost = arr[i].posts;
        name = arr[i].profile;
      }
    }
